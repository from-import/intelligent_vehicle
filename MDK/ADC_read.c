...
void ADC_read()
{
    ...
    /*
    data_last[0] = data_last[0] > AD_1MAX ? AD_1MAX : data_last[0];
    data_last[1] = data_last[1] > AD_2MAX ? AD_2MAX : data_last[1];
    data_last[3] = data_last[3] > AD_4MAX ? AD_4MAX : data_last[3];
    data_last[4] = data_last[4] > AD_5MAX ? AD_5MAX : data_last[4];

    data_last[0] = data_last[0] < AD_1MIN ? AD_1MIN : data_last[0];
    data_last[1] = data_last[1] < AD_2MIN ? AD_2MIN : data_last[1];
    data_last[3] = data_last[3] < AD_4MIN ? AD_4MIN : data_last[3];
    data_last[4] = data_last[4] < AD_5MIN ? AD_5MIN : data_last[4];
    */

    data_last[0] = 100 *(data_last[0] - AD_1MIN)  / (AD_1MAX - AD_1MIN);
    data_last[1] = 100 *(data_last[1] - AD_2MIN)  / (AD_2MAX - AD_2MIN);
    data_last[3] = 100 *(data_last[3] - AD_4MIN)  / (AD_4MAX - AD_4MIN);
    data_last[4] = 100 *(data_last[4] - AD_5MIN)  / (AD_5MAX - AD_5MIN);
    ...
    rightOrLeft = ad_sum > 0 ? 2:1; // rightOrLeft 表示应该往哪个方向拐弯

		// 差比和加权算法
    offset = (int)100 * ((left_value - right_value) / (left_value + right_value)); // 含竖电感的差值
    offset2 = 100 * (data_last[0] - data_last[4]) / (data_last[0] + data_last[4]); // 仅有横电感的差值
