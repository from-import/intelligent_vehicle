...
{
	/*****************************************************************
	Tips:Type=0  为直线状态 // Go Long
	     Type=1  为转弯状态 // turn 
			 Type=2  为R入环状态  // RIGHT GO IN  CIRCLE
			 Type=3  为L入环状态 	// LEFT GO IN CIRCLE
			 Type=4  为避障状态 // Avoid Obstacles
	*****************************************************************/	
...
//		Type = 0;
		if((data_last[1] >  200 || data_last[3] > 200) && data_last[2] < 300)	//转弯（在转弯条件下0会出现700+的值）
	    {Type = 1;multiple = 1;}
